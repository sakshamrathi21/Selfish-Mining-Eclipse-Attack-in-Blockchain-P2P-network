...
def run_simulation(num_peer=100, percent_malicious=50, Ttx=10, Tk=100, get_timeout=20, total_time=10000, iterations=2):
    print(f"Running simulation with num_peer={num_peer}, percent_malicious={percent_malicious}, Ttx={Ttx}, Tk={Tk}, get_timeout={get_timeout}, total_time={total_time} --no-eclipse", flush=True)

    output_filename = f"ringmaster_{num_peer}_{percent_malicious}_{Ttx}_{Tk}_{get_timeout}_{total_time}_no_eclipse.png"

    if os.path.exists(output_filename):
        print("Already calculated. Skipping...")
        return
    outputs = [subprocess.run(["./run", str(num_peer), str(percent_malicious), str(Ttx), str(Tk), str(get_timeout), str(total_time), "--ratio", "--blockchain", "--no-eclipse"], 
